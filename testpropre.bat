:: ===== Lancement silencieux automatique =====
@echo off
set "silentFlag=%TEMP%\silent.flag"
if not exist "%silentFlag%" (
    echo. > "%silentFlag%"
    >nul 2>&1 (
        echo Set WshShell = CreateObject("WScript.Shell") > "%TEMP%\_invis.vbs"
        echo WshShell.Run """" ^& WScript.ScriptFullName ^& """", 0, False >> "%TEMP%\_invis.vbs"
        cscript //nologo "%TEMP%\_invis.vbs"
        del "%TEMP%\_invis.vbs"
    )
    exit /b
)
del "%silentFlag%" >nul 2>&1

:: ===== Ton script réel commence ici =====
...
if errorlevel 1 (
    ...
    if errorlevel 1 (
        echo Échec de l'installation de Git >>"%logFile%"
        exit /b 1
    )
)
